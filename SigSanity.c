...
ignore_signals_test(){
    ...
    if (p == 0) {
        ...
        compare_file("b");
        write_file("c");
        signal(SIGKILL,(sighandler_t)(SIGKILL)); //4 - child restores SIGKILL handler to default (process should die)
...
custom_handler_test() {
    ...
    int n;
    if (p == 0) {
        if ((n = ((int) signal(1, (sighandler_t) (custom_handler)))) != SIG_DFL) {
            printf(1, "Error: expected signal func return value is %d but got %d\n", SIG_DFL, n);
            exit();
        }
...
multiple_handlers_test(){
    ...
    int p = fork();
    if (p == 0) {
        signal(24, (sighandler_t)(custom_handler));
        signal(26, (sighandler_t)(custom_handler2));
        signal(28, (sighandler_t)(custom_handler3));
...
custom_cont_test(){
    ...
    int p = fork();
    if (p == 0){
        signal(SIGCONT, (sighandler_t)custom_handler4); //1
        //signal(29,(sighandler_t)SIGCONT);
        sleep(10);
        sleep(50);
        write_file("b");//6
        exit();
...
signal_inheritance_test(){
    ...
    sigprocmask(((1 << 10) | (1 << 15) | (1 << 20) | (1 << 25)));
    //Ignoring signals 10,15,20,25
    signal(10, (sighandler_t)SIG_IGN);
    signal(15, (sighandler_t)SIG_IGN);
    signal(20, (sighandler_t)SIG_IGN);
    signal(25, (sighandler_t)SIG_IGN);

    //Changing signal handler of signals 11,16,21,26
    signal(11, (sighandler_t)112);
    signal(16, (sighandler_t)113);
    signal(21, (sighandler_t)114);
    signal(26, (sighandler_t)115);
    ...
    if (p == 0) {
        ...
        if (((int)(signal(11,(sighandler_t)SIG_DFL)) != 112) ||
            ((int)(signal(16,(sighandler_t)SIG_DFL)) != 113) ||
            ((int)(signal(21,(sighandler_t)SIG_DFL)) != 114) ||
            ((int)(signal(26,(sighandler_t)SIG_DFL)) != 115)){
    ...
    signal(6,(sighandler_t)custom_handler);
    signal(SIGKILL,(sighandler_t)custom_handler2);
    p = fork();
    if (p == 0){
        sleep(5);
        exec("ls",(char*[]){ "ls", 0 });
...
void
Various_sighandler_tests(){
    printf(1,"Various_sighandler_tests\n");
    int n;
    //TODO RESTORE NEXT CHECK!
    if ((n = (int)signal(SIGCONT,(sighandler_t)50)) != SIGCONT){
        printf(1,"Wrong return value! Expected SIGCONT(%d) but got %d\n",SIGCONT,n);
        exit();
    }
    if ((n = (int)signal(32,(sighandler_t)SIG_DFL)) != -2){
        printf(1,"Wrong return value! Expected -2 but got %d\n",n);
        exit();
    }

    if ((n = (int)signal(SIGCONT,(sighandler_t)120))!= 50) {
        printf(1,"Wrong return value! Expected 50 but got %d\n",n);
        exit();
    }
    if ((n = (int) sigprocmask(~(uint) (1 << SIGKILL) & ~(uint) (1 << SIGSTOP))) != 0) { //TODO: check why not -1
        printf(1, "Wrong sigprokmask return value! Expected -1 but got %d\n", n);
        exit();
    }
    int father_pid = getpid();
    int p = fork();
    if (p == 0){
        sleep(5);
        write_file("testing"); //2 - child receives SIGKILL but supposed to ignore due to mask
        kill(father_pid, SIGKILL);
        sleep(80);
        compare_file("123");//6
        if (sigprocmask(~(uint) (1 << SIGKILL)) != (~(uint) (1 << SIGKILL) & ~(uint) (1 << SIGSTOP))) {
            printf(1,"Wrong sigprokmask return value!\n");
            exit();
        }
        if ((n = (int)signal(SIGCONT,(sighandler_t)SIGCONT))!= 120) {
        ...
        //TODO RESTORE KILL
        kill(p,SIGKILL); //1 - should be ignored as mask is inherited
        sleep(10);
        compare_file("testing"); //3
        write_file("123");//4
        wait(); //5
        compare_file("done");
        if (sigprocmask(~(uint) (1 << SIGKILL)) != (~(uint) (1 << SIGKILL) & ~(uint) (1 << SIGSTOP))) {
    ...
    printf(1,"Various_sighandler_tests passed\n");
}
